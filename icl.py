...
def get_icl_prompts(
    ...
    """
    prompt = ''
    idxs = list(range(len(support_inputs)))
    random.shuffle(idxs)
    if prompt_mode == 'qa':
        # if not support_labels[i].endswith('.'):
        #     support_labels[i] += '.'
        prompt += ''.join([support_inputs[i] + ' In the ' + support_labels[i] for i in idxs]) + test_input + ' In the'
    elif prompt_mode == 'none':
        prompt += ''.join([support_inputs[i] + ' ' + support_labels[i] + ' ' for i in idxs] ) + test_input
    elif prompt_mode == 'tldr':
        prompt += ''.join([support_inputs[i] + ' TL;DR ' + support_labels[i] + ' ' for i in idxs]) + test_input
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    with torch.inference_mode():
        while len(sampled_tokens) < max_tokens:
            ...
            input_ids = torch.argmax(output.logits[-1, -1, :], -1)
            if input_ids.item() in stop_tokens:
                break
            else:
                sampled_tokens.append(input_ids.item())
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            decoded_prediction = tokenizer.decode(sampled_tokens).strip()
